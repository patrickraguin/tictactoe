...
void main() {
  group('playMove', () {
    test('advances turn on a valid move', () {
      ...
      final next = playMove(state: state, index: 0, mark: CellMarkEnum.x);
    ...
    test('rejects move on an occupied cell', () {
      ...
      final next = playMove(state: state, index: 0, mark: CellMarkEnum.o);
      expect(next, state);
    });

    test('rejects move when not player\'s turn', () {
      final state = initialState(
        board: BoardEntity.empty(),
        firstToPlay: CellMarkEnum.x,
        humanMark: CellMarkEnum.x,
    ...
    test('transitions to WonEntity when move completes a line', () {
      ...
      final next = playMove(state: state, index: 2, mark: CellMarkEnum.x);
    ...
    test('transitions to DrawEntity when board fills without winner', () {
      ...
      final next = playMove(state: state, index: 8, mark: CellMarkEnum.x);
      expect(next, isA<DrawEntity>());
    });
  });
}
